...
namespace Way.Lib
{
    ...
    /// 证书生成器
    /// </summary>
    public sealed class CertificateMaker
    {
        #region 生成证书
        /// <summary>   
        ...
        /// <param name="makecertPath">makecert.exe文件路径，C:\Program Files (x86)\Windows Kits\8.1\bin\x64\makecert.exe</param>   
        /// <returns></returns>   
        ...
        /// <returns></returns>
        public static bool CreateCertWithPrivateKey(string subjectName, string makecertPath,DateTime startTime,DateTime endTime)
        {
            subjectName = "CN=" + subjectName;
            string param = "-r -pe -n \"" + subjectName + "\" -b "+ startTime.ToString("dd/MM/yyyy") + " -e " + endTime.ToString("dd/MM/yyyy") + " -sky exchange -ss my";
            try
            {
                Process p = Process.Start(makecertPath, param);
                p.WaitForExit();
