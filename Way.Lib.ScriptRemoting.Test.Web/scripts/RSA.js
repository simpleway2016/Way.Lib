...
function decryptedItem(key, s) {
    ...
    if (key.radix == 16) {
        ...
        bi = biFromString(s, key.radix);
    }
    block = key.barrett.powMod(bi, key.d);
    for (j = 0; j <= biHighIndex(block) ; ++j) {
        var str = String.fromCharCode(block.digits[j] >> 8, block.digits[j] & 255);
...
function decryptedString(key, s) {
    var result = "";
    for (var i = 0 ; i < s.length ; i += 256) {
        result += decryptedItem(key, s.substr(i, 256));
    }
    return result;
}