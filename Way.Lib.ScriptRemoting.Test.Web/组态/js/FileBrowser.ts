...
class FileBrowser
{
    ...
    constructor()
    {
        ...
        this.fileElement = <HTMLElement>toolDiv.children[3];
        this.fileElement.addEventListener("change", () => {
            toolDiv.children[2].innerHTML = ((<any>this.fileElement).value);
        }, false);
        toolDiv.children[0].addEventListener("click", () => { this.addFolderDialog(); }, false);
        toolDiv.children[4].addEventListener("click", () => { this.updateFile(); }, false);
        toolDiv.children[5].addEventListener("click", () => {
            this.hide();
           
        }, false);
        toolDiv.children[6].addEventListener("click", () => { if (this.onSelectFile) { this.onSelectFile(""); } this.hide(); }, false);
