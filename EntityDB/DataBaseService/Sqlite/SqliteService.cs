...
namespace EntityDB
{
    ...
    class SqliteService:IDatabaseService
    {
        ...
        protected virtual System.Data.Common.DbCommand CreateCommand(string sql)
        {
            ...
            try
            {
                var command = CreateCommand(sql);
                {
                    if (parames != null && parames.Length > 0)
                    {
                        ...
                        {
                            command.Parameters.Add(CreateParameter("@p" + i, parames[i]));
                        }
                    }
        ...
        public DataTable SelectTable(string sql, params object[] parames)
        {
            bool needToClose = false;
            if (this.Connection.State != System.Data.ConnectionState.Open)
            {
            ...
            try
            {
                using (var dataAdapter = CreateDataAdapter(sql))
                {
                    if (parames != null && parames.Length > 0)
                    {
                        for (int i = 0; i < parames.Length; i++)
                        {
                            dataAdapter.SelectCommand.Parameters.Add(CreateParameter("@p" + i, parames[i]));
                        }
                    }
        ...
        public DataSet SelectDataSet(string sql, params object[] parames)
        {
            bool needToClose = false;
            if (this.Connection.State != System.Data.ConnectionState.Open)
            {
        ...
        public virtual DataTable SelectTable(string sql, int skip, int take, params object[] sqlparameters)
        {
